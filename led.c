...
{
    cy_rslt_t rslt;
    cyhal_clock_t pwm_clock;

	/* Initialize clock for the ADC Block.*/
    rslt = cyhal_clock_allocate(&pwm_clock, CYHAL_CLOCK_BLOCK_PERIPHERAL_8BIT);
    if(rslt == CY_RSLT_SUCCESS)
    {
    	rslt = cyhal_clock_set_enabled(&pwm_clock, true, true);
    	rslt = cyhal_pwm_init(&pwm_led, LED1, &pwm_clock);
    }
